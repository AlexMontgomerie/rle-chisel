...
class DecoderTest extends FlatSpec with ChiselScalatestTester with Matchers {
  ...
  def run_test (seq_in: Seq[UInt], seq_out: Seq[UInt], description: String) {
    ...
    it should s"be correct for $description (Decoder)" in {
      // create the DUT
      test(new Decoder(UInt(8.W), 0)).withAnnotations(annotations) { c =>
  ...
  seq_in = Seq(1.U, 2.U, 0.U, 4.U, 7.U, 8.U, 9.U)
  seq_out = Seq(1.U, 2.U, 0.U, 0.U, 0.U, 0.U, 7.U, 8.U, 9.U)
  run_test(seq_in, seq_out, description)

  description = "a stream with a single zero"
  seq_in = Seq(1.U, 2.U, 0.U, 1.U, 7.U, 8.U, 9.U)
  seq_out = Seq(1.U, 2.U, 0.U, 7.U, 8.U, 9.U)
  run_test(seq_in, seq_out, description)

  description = "a stream with a single zero at the end"
  seq_in = Seq(1.U, 2.U, 7.U, 8.U, 9.U, 0.U, 1.U)
  seq_out = Seq(1.U, 2.U, 7.U, 8.U, 9.U, 0.U)
  run_test(seq_in, seq_out, description)
